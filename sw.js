...
// ==========================================
const CACHE_NAME = 'zonevault-v252'; // BUMPED VERSION TO FORCE REFRESH (v250 -> v251)
const urlsToCache = [
  './',
  './index.html',
  './home.html',
...
self.addEventListener('fetch', event => {
  ...
  // ==========================================================
  // STRATEGY: Network-Only for cross-origin gallery images
  // (Cloudinary / GitHub raw). These MUST stay a clean, direct
  // 'cors' passthrough fetch. If these fall into the Cache-First
  // branch below, the SW's internal fetch can end up returning an
  // opaque response, which breaks the page's downloadImage() function
  // (fetch(src, { mode: 'cors' })) -> the blob download always fails
  // and falls back to window.open(), i.e. the image just opens in a
  // new tab instead of actually saving to the device.
  // ==========================================================
  if (
  ...
  if (request.url.includes('.js') || request.url.includes('.html') || request.mode === 'navigate') {
    ...
    // STRATEGY: Cache-First for local Images/CSS (Para mabilis ang load)
    event.respondWith(
